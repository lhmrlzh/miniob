  ...
  if (child_oper->type() == LogicalOperatorType::TABLE_GET) {
    ...
    if (left_join_child_type == LogicalOperatorType::JOIN && right_join_child_type == LogicalOperatorType::TABLE_GET) {
      ...
      for (auto it = pushdown_exprs.begin(); it != pushdown_exprs.end();) {
        auto comparison_exper = static_cast<ComparisonExpr *>((*it).get());

        bool related_to_right_table = false;
        if (comparison_exper->left()->type() == ExprType::FIELD) {
          auto left_expr         = static_cast<FieldExpr *>(comparison_exper->left().get());
          related_to_right_table = related_to_right_table || (strcmp(left_expr->table_name(), right_table_name) == 0);
        }
        if (comparison_exper->right()->type() == ExprType::FIELD) {
          auto right_expr        = static_cast<FieldExpr *>(comparison_exper->right().get());
      ...
      std::unique_ptr<ConjunctionExpr> conjuction_expr(new ConjunctionExpr(ConjunctionExpr::Type::AND, pushdown_exprs));
      std::unique_ptr<PredicateLogicalOperator> left_new_oper =
          std::unique_ptr<PredicateLogicalOperator>(new PredicateLogicalOperator(std::move(conjuction_expr)));
